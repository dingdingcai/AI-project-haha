...
class MyImageDataGenerator(ImageDataGenerator):
    ...
    def myflow_from_directory(self, directory,
        ...
        return MyDirectoryIterator(
            ...
            return_index_array=return_index_array,
...
class MyDirectoryIterator(Iterator):
    ...
    def __init__(self, directory,
        ...
        self.return_index_array = return_index_array
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        batch_mask = np.zeros((len(index_array), self.heatmap_height, self.heatmap_width,
                               self.heatmap_height, self.heatmap_width, 1), dtype=np.uint8)
        batch_similarity = np.zeros_like(batch_mask)
        ...
        for i, j in enumerate(index_array):
            ...
            a_sku_points = dict([(k, str2float(v)) for k, v in a_mp.items() if len(v) > 0])
            b_sku_points = dict([(k, str2float(v)) for k, v in b_mp.items() if len(v) > 0])
            img_path.append([a_path, b_path])
            
#             data = self.basic_directory_dict[j]
#             a_sku_points, b_sku_points = data['a_data'], data['b_data']
            # load img
            img_a = load_img(os.path.join(a_path), grayscale=grayscale, target_size=self.target_size,
                             interpolation=self.interpolation)
            a = img_to_array(img_a, data_format=self.data_format)
            img_b = load_img(os.path.join(b_path), grayscale=grayscale, target_size=self.target_size,
                             interpolation=self.interpolation)
            ...
            a_sku_points = dict([(k, transform_point(v, params)) for k, v in a_sku_points.items()])
            b_sku_points = dict([(k, transform_point(v, params)) for k, v in b_sku_points.items()])
            for a_sku, a_point_list in a_sku_points.items():
                for b_sku, b_point_list in b_sku_points.items():
                    for b_point in b_point_list:
                        h_b, w_b = b_point[1], b_point[0]
                        if 0 < h_b < 1 and 0 < w_b < 1:
                            h_b, w_b = int(h_b * self.heatmap_height), int(w_b * self.heatmap_width)
                            # same sku, so set similarity as 1
                            for a_point in a_point_list:
                                h_a, w_a = a_point[1], a_point[0]
                                if 0 < h_a < 1 and 0 < w_a < 1:
                                    h_a, w_a = int(h_a * self.heatmap_height), int(w_a * self.heatmap_width)
                                    # do with similarity
                                    if b_sku == a_sku:
                                        batch_similarity[i, h_b, w_b, h_a, w_a, :] = 1
                                    # do with mask
                                    batch_mask[i, h_b, w_b, h_a, w_a, :] = 1
            ...
            batch_img_a[i] = a
            batch_img_b[i] = b
            a_return.append(a_sku_points)
            b_return.append(b_sku_points)
            
            a_mp = merge_double_list(a_sku_points.values())
            b_mp = merge_double_list(b_sku_points.values())
        ...
        if self.save_to_dir:
            for i, j in enumerate(index_array):
                img_a = array_to_img(batch_img_a[i], self.data_format, scale=True)
                fname_a = '{prefix}_{index}_{hash}.{format}'.format(prefix='a' + self.save_prefix,
                                                                    index=j,
                                                                    hash=np.random.randint(1e7),
                                                                    format=self.save_format)
                img_a.save(os.path.join(self.save_to_dir, fname_a))
                img_b = array_to_img(batch_img_b[i], self.data_format, scale=True)
                fname_b = '{prefix}_{index}_{hash}.{format}'.format(prefix='b' + self.save_prefix,
                                                                    index=j,
                                                                    hash=np.random.randint(1e7),
                                                                    format=self.save_format)
                img_b.save(os.path.join(self.save_to_dir, fname_b))
        if not self.return_index_array:
            return [batch_img_a, batch_img_b, batch_mask], batch_similarity
        else:
            return [batch_img_a, batch_img_b, batch_mask], batch_similarity, index_array, a_return, b_return, img_path, batch_input_a_all, batch_input_b_all
...
def gather_data(directory, simple_take_flag = False, follow_links=True):
    # init basic directory dict
    basic_directory_dict = {}
    data_num = 0
    # fisrt level
    ...
    for dirname in dirnames:
        ...
        for second_dirname in second_dirnames:
            ...
            x_dirpath = os.path.join(directory, third_dirpath)
            begin_flag = False
            for x_filename in third_filenames:
                if x_filename.endswith('.jpg'):
                    ...
                    if read_json(data_path) is None:
                        ...
                        if begin_flag:
                            if not simple_take_flag or now_num < before_num:
                                valid_filenames.append(x_filename)
                        else:
                            begin_flag = True
                        before_num = now_num
            data_num += len(valid_filenames)
            basic_directory_dict[x_dirpath] = valid_filenames
    return data_num, basic_directory_dict
